...
strategy("AI MNQ Smart Paper Bot v9", overlay=true, initial_capital=100000, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=0, process_orders_on_close=true, calc_on_every_tick=true)

contractQty = input.int(1, "Contracts / Paper Size", minval=1, maxval=10)
mnqDollarsPerPoint = input.float(2.0, "MNQ Dollars Per Point", minval=0.01, step=0.25)
...
webhookTag = input.string("mnq-smart-paper-v9", "Webhook Tag")
...
riskCapPoints = math.max(maxLossDollars / math.max(contractQty * mnqDollarsPerPoint, 0.01), syminfo.mintick * 4)
minimumStopDistance = math.max(atrValue * minRiskAtr, syminfo.mintick * 4)
minimumTargetDistance = minimumProfitTargetDollars / math.max(contractQty * mnqDollarsPerPoint, 0.01)
...
longTakeProfit = close + math.max(longRisk * takeProfitR, minimumTargetDistance)
shortTakeProfit = close - math.max(shortRisk * takeProfitR, minimumTargetDistance)
...
breakEvenBufferPoints = breakEvenBufferDollars / math.max(contractQty * mnqDollarsPerPoint, 0.01)
...
if barstate.islast
    table.cell(statsTable, 0, 0, "MNQ Bot v9", text_color=color.white, bgcolor=color.new(color.blue, 72))
