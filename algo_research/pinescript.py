...
strategy("AI MNQ Smart Paper Bot v11", overlay=true, initial_capital=50000, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=0, process_orders_on_close=true, calc_on_every_tick=true)
...
webhookTag = input.string("mnq-smart-paper-v11", "Webhook Tag")
...
safeContractQty = math.min(contractQty, maxContractsForAccount)
tradeQtyForRisk = math.max(safeContractQty, 1)
sizeOk = safeContractQty >= 1 and notionalPerContract * safeContractQty <= accountSizeDollars
...
if barstate.islast
    table.cell(statsTable, 0, 0, "MNQ Bot v11", text_color=color.white, bgcolor=color.new(color.blue, 72))
