...
strategy("AI NQ1 Smart Paper Bot v44 - Pro Simple", overlay=true, initial_capital=100000, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=0, margin_long=10, margin_short=10, process_orders_on_close=false, calc_on_every_tick=true, calc_on_order_fills=true, commission_type=strategy.commission.cash_per_contract, commission_value=4.0, slippage=2)
...
rewardMultiple = input.float(2.2, "Reward Multiple", minval=0.5, maxval=10.0, step=0.25)
trailAfterR = input.float(0.9, "Trail After R", minval=0.25, maxval=5.0, step=0.25)
trailAtrMult = input.float(1.35, "Trail ATR Multiplier", minval=0.25, maxval=5.0, step=0.05)
maxBarsInTrade = input.int(8, "Max Weak Bars In Trade", minval=2, maxval=50)
...
plannedLongStopRaw = enableManualTest ? signalClose - atrValue : math.max(longAtrStop, longStructureStop)
plannedShortStopRaw = enableManualTest ? signalClose + atrValue : math.min(shortAtrStop, shortStructureStop)
...
longSetup = kaufmanLong ? "KaufmanRatio" : atrLong ? "ATRExpansion" : emaPullbackLong ? "EMAPullback" : sweepLong ? "EMAPullback" : "ManualTest"
shortSetup = kaufmanShort ? "KaufmanRatio" : atrShort ? "ATRExpansion" : emaPullbackShort ? "EMAPullback" : sweepShort ? "EMAPullback" : "ManualTest"
longGrade = longScore >= 84 ? "A+" : longScore >= 74 ? "A" : "B"
shortGrade = shortScore >= 84 ? "A+" : shortScore >= 74 ? "A" : "B"
longReason = longSetup + " | score " + str.tostring(longScore, "#") + " | KR " + str.tostring(kaufmanRatioValue, "#.##") + " | ATR " + str.tostring(atrRatio, "#.##") + "x | chop " + str.tostring(chopScore, "#") + " | VWAP/location/no-repaint"
shortReason = shortSetup + " | score " + str.tostring(shortScore, "#") + " | KR " + str.tostring(kaufmanRatioValue, "#.##") + " | ATR " + str.tostring(atrRatio, "#.##") + "x | chop " + str.tostring(chopScore, "#") + " | VWAP/location/no-repaint"

longMessage = '{"source":"tradingview","action":"BUY","ticker":"' + syminfo.ticker + '","timeframe":"' + signalTimeframe + '","chart_timeframe":"' + timeframe.period + '","price":' + money(signalClose) + ',"target":' + money(plannedLongTarget) + ',"profit_mode":"BRACKET_TARGET","stop_mode":"HARD_STOP","stop":' + money(plannedLongStop) + ',"qty":' + str.tostring(qty) + ',"edge":' + str.tostring(edge, "#.##") + ',"score":' + str.tostring(longScore, "#.##") + ',"ai_score":' + str.tostring(longScore, "#.##") + ',"ai_bias":"LONG","entry_style":"' + longSetup + '","grade":"' + longGrade + '","risk_mode":"ProSimple","reason":"' + longReason + '","bar_time":"' + str.tostring(signalTime) + '","tag":"' + webhookTag + '"}'
shortMessage = '{"source":"tradingview","action":"SELL","ticker":"' + syminfo.ticker + '","timeframe":"' + signalTimeframe + '","chart_timeframe":"' + timeframe.period + '","price":' + money(signalClose) + ',"target":' + money(plannedShortTarget) + ',"profit_mode":"BRACKET_TARGET","stop_mode":"HARD_STOP","stop":' + money(plannedShortStop) + ',"qty":' + str.tostring(qty) + ',"edge":' + str.tostring(edge, "#.##") + ',"score":' + str.tostring(shortScore, "#.##") + ',"ai_score":' + str.tostring(shortScore, "#.##") + ',"ai_bias":"SHORT","entry_style":"' + shortSetup + '","grade":"' + shortGrade + '","risk_mode":"ProSimple","reason":"' + shortReason + '","bar_time":"' + str.tostring(signalTime) + '","tag":"' + webhookTag + '"}'
exitLongMessage = '{"source":"tradingview","action":"EXIT_LONG","ticker":"' + syminfo.ticker + '","timeframe":"' + signalTimeframe + '","chart_timeframe":"' + timeframe.period + '","price":' + money(signalClose) + ',"qty":' + str.tostring(qty) + ',"edge":' + str.tostring(edge, "#.##") + ',"score":' + str.tostring(longScore, "#.##") + ',"reason":"ProSimple thesis exit","bar_time":"' + str.tostring(signalTime) + '","tag":"' + webhookTag + '"}'
exitShortMessage = '{"source":"tradingview","action":"EXIT_SHORT","ticker":"' + syminfo.ticker + '","timeframe":"' + signalTimeframe + '","chart_timeframe":"' + timeframe.period + '","price":' + money(signalClose) + ',"qty":' + str.tostring(qty) + ',"edge":' + str.tostring(edge, "#.##") + ',"score":' + str.tostring(shortScore, "#.##") + ',"reason":"ProSimple thesis exit","bar_time":"' + str.tostring(signalTime) + '","tag":"' + webhookTag + '"}'
...
if strategy.position_size > 0 and signalReady
    ...
    if openProfitPoints >= activeRiskPoints * trailAfterR
        ...
        activeStop := math.max(math.max(activeStop, structureTrail), strategy.position_avg_price)
...
if strategy.position_size < 0 and signalReady
    ...
    if openProfitPoints >= activeRiskPoints * trailAfterR
        structureTrail = signalClose + atrValue * trailAtrMult
        activeStop := math.min(math.min(activeStop, structureTrail), strategy.position_avg_price)
    strategy.exit("Short Risk Exit", from_entry="Short", stop=activeStop, limit=activeTarget, alert_message=exitShortMessage)

longThesisExit = strategy.position_size > 0 and confirmedOk and newSignalBar and ((signalClose < emaPullback and longHealth < 58) or signalClose < vwapValue and longHealth < 52 or shortScore > longScore + 12 or barsInTrade >= maxBarsInTrade and longHealth < 62 or testExitLongSignal)
shortThesisExit = strategy.position_size < 0 and confirmedOk and newSignalBar and ((signalClose > emaPullback and shortHealth < 58) or signalClose > vwapValue and shortHealth < 52 or longScore > shortScore + 12 or barsInTrade >= maxBarsInTrade and shortHealth < 62 or testExitShortSignal)
