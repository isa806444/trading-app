...
contractQty = input.int(1, "Tradovate Contracts", minval=1)
...
longTargetPrice = close * (1 + targetPct)
longStopPrice = close * (1 - stopPct)
shortTargetPrice = close * (1 - targetPct)
shortStopPrice = close * (1 + stopPct)
longMessage = '{"source":"tradingview","action":"BUY","ticker":"' + syminfo.ticker + '","price":' + str.tostring(close, format.mintick) + ',"target":' + str.tostring(longTargetPrice, format.mintick) + ',"stop":' + str.tostring(longStopPrice, format.mintick) + ',"qty":' + str.tostring(contractQty) + ',"edge":' + str.tostring(edge) + ',"bar_time":"' + str.tostring(time) + '","tag":"' + webhookTag + '"}'
shortMessage = '{"source":"tradingview","action":"SELL","ticker":"' + syminfo.ticker + '","price":' + str.tostring(close, format.mintick) + ',"target":' + str.tostring(shortTargetPrice, format.mintick) + ',"stop":' + str.tostring(shortStopPrice, format.mintick) + ',"qty":' + str.tostring(contractQty) + ',"edge":' + str.tostring(edge) + ',"bar_time":"' + str.tostring(time) + '","tag":"' + webhookTag + '"}'

if longSignal and strategy.position_size <= 0
    strategy.entry("AI Long", strategy.long, qty=contractQty, alert_message=longMessage)
    alert(longMessage, alert.freq_once_per_bar)
if shortSignal and strategy.position_size >= 0
    strategy.entry("AI Short", strategy.short, qty=contractQty, alert_message=shortMessage)
