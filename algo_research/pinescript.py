...
emaPullbackLen = input.int(21, "Reversal EMA", minval=5, maxval=100)
...
pullbackAtrBuffer = input.float(0.35, "Reversal Level ATR Buffer", minval=0.05, maxval=2.0, step=0.05)
...
reversalLong = (sweepLong or reclaimLong or (signalLow <= emaPullback + atrValue * pullbackAtrBuffer and signalClose > emaPullback)) and signalClose > signalOpen and closeNearHigh and lowerWick > upperWick and volumeOk and (not useHtfBias or htfBull or sweepLong)
reversalShort = (sweepShort or rejectShort or (signalHigh >= emaPullback - atrValue * pullbackAtrBuffer and signalClose < emaPullback)) and signalClose < signalOpen and closeNearLow and upperWick > lowerWick and volumeOk and (not useHtfBias or htfBear or sweepShort)
...
longScore = clamp(42.0 + scoreBool(bullTrend, 12) + scoreBool(htfBull, 10) + scoreBool(signalClose > vwapValue, 8) + scoreBool(vwapSlope > 0, 5) + scoreBool(volumeOk, 7) + scoreBool(relativeVolume >= 1.1, 5) + scoreBool(kaufmanRatioValue >= minKaufmanRatio, 8) + scoreBool(trendStrength >= minTrendStrength, 6) + scoreBool(closeNearHigh, 5) + scoreBool(reclaimLong or sweepLong, 8) + scoreBool(reversalLong, 9) + scoreBool(breakLong, 6) - scoreBool(chopScore > maxChopScore, 12) - scoreBool(fakeoutLongRisk, 12) - scoreBool(doNotChaseLong, 10) - scoreBool(bearTrend and htfBear, 12), 0.0, 100.0)
...
chopOk = chopScore <= maxChopScore or reversalLong or reversalShort
...
newLongSignal = baseGate and strategy.position_size == 0 and longScore >= adaptiveMinScore and longScore >= shortScore + 4 and not fakeoutLongRisk and not doNotChaseLong and (kaufmanLong or atrLong or reversalLong)
newShortSignal = baseGate and strategy.position_size == 0 and shortScore >= adaptiveMinScore and shortScore >= longScore + 4 and not fakeoutShortRisk and not doNotChaseShort and (kaufmanShort or atrShort or reversalShort)
...
longSetup = enableHourlyWebhookTest ? "HourlyWebhookTest" : kaufmanLong ? "KaufmanRatio" : atrLong ? "ATRExpansion" : reversalLong ? "Reversal" : "ManualTest"
shortSetup = enableHourlyWebhookTest ? "HourlyWebhookTest" : kaufmanShort ? "KaufmanRatio" : atrShort ? "ATRExpansion" : reversalShort ? "Reversal" : "ManualTest"
...
plot(emaPullback, "Reversal EMA", color=color.new(color.white, 0), linewidth=1)
...
setupText = kaufmanLong or kaufmanShort ? "Kaufman" : atrLong or atrShort ? "ATR" : reversalLong or reversalShort ? "Reversal" : "Waiting"
