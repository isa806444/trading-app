...
dailyFallbackScore = input.float(60.0, "Pace Quality Minimum Score", minval=45.0, maxval=100.0, step=1.0)
dailyFallbackEdge = input.float(4.0, "Pace Quality Minimum Edge", minval=0.0, step=1.0)
...
sidewaysMarket = adxValue < 13 and trendEfficiency < 0.16 and compressionRange <= 2.60 and not bullCoilBreak and not bearCoilBreak and not bullLiquiditySweep and not bearLiquiditySweep
...
paceConfirmMin = behindTradePace ? 3 : 4
firstTradeScoreMin = dailyFallbackScore - 10
firstTradeConfirmMin = 2
...
dailyLongSafety = validLongRisk and notExtremeChase and (close >= emaFast or close >= vwapValue or plusDi >= minusDi or bullOpportunityPattern)
dailyShortSafety = validShortRisk and notExtremeChase and (close <= emaFast or close <= vwapValue or minusDi >= plusDi or bearOpportunityPattern)
longOpportunityQuality = bullOpportunityPattern or bullImpulse or bullTape or cleanTrend
shortOpportunityQuality = bearOpportunityPattern or bearImpulse or bearTape or cleanTrend
longFirstDailyCandidate = opportunityReady and canEnter and firstTradeWindow and dailyLongSafety and longOpportunityQuality and longConfirmations >= firstTradeConfirmMin and buyScore >= firstTradeScoreMin and fallbackLongBias and (edge >= 0 or bullOpportunityPattern)
shortFirstDailyCandidate = opportunityReady and canEnter and firstTradeWindow and dailyShortSafety and shortOpportunityQuality and shortConfirmations >= firstTradeConfirmMin and sellScore >= firstTradeScoreMin and fallbackShortBias and not longFirstDailyCandidate and (edge <= 0 or bearOpportunityPattern)
...
longMinimumQuality = bullOpportunityPattern or bullImpulse or cleanTrend or bullTape
shortMinimumQuality = bearOpportunityPattern or bearImpulse or cleanTrend or bearTape
