...
strategy("AI MNQ Smart Paper Bot v12", overlay=true, initial_capital=50000, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=0, process_orders_on_close=true, calc_on_every_tick=true)

contractQty = input.int(1, "Requested Contracts / Paper Size", minval=1, maxval=10)
accountSizeDollars = input.float(50000.0, "Account Size Guard ($)", minval=1000.0, step=1000.0)
...
webhookTag = input.string("mnq-smart-paper-v12", "Webhook Tag")
...
dailyFallbackScore = input.float(66.0, "Pace Quality Minimum Score", minval=45.0, maxval=100.0, step=1.0)
...
paceTargetByNow = nyHour <= 10 ? 1 : nyHour <= 11 ? 2 : nyHour <= 12 ? 3 : nyHour <= 14 ? 4 : 5
...
paceScoreMin = dailyFallbackScore + (behindTradePace ? 0.0 : 8.0)
paceEdgeMin = dailyFallbackEdge + (behindTradePace ? 0.0 : 4.0)
...
if barstate.islast
    table.cell(statsTable, 0, 0, "MNQ Bot v12", text_color=color.white, bgcolor=color.new(color.blue, 72))
