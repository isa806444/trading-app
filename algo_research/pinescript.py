...
strategy("AI MNQ Smart Paper Bot v15", overlay=true, initial_capital=100000, default_qty_type=strategy.fixed, default_qty_value=1, pyramiding=0, process_orders_on_close=true, calc_on_every_tick=true)
...
webhookTag = input.string("mnq-smart-paper-v15", "Webhook Tag")
...
opportunityReady = barstate.isconfirmed and symbolOk and timeframeOk and sessionOk and withinBacktestWindow and tradeableTape and notExtremeChase and atrValue > syminfo.mintick * 4 and not na(priorHigh) and not na(priorLow)
...
longDailyCandidate = opportunityReady and canEnter and dailyMode and dailyLongSafety and longOpportunityQuality and longConfirmations >= paceConfirmMin and buyScore >= paceScoreMin and edge >= paceEdgeMin
shortDailyCandidate = opportunityReady and canEnter and dailyMode and dailyShortSafety and shortOpportunityQuality and shortConfirmations >= paceConfirmMin and sellScore >= paceScoreMin and edge <= -paceEdgeMin
...
if barstate.islast
    table.cell(statsTable, 0, 0, "MNQ Bot v15", text_color=color.white, bgcolor=color.new(color.blue, 72))
